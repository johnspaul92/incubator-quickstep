...
class LIPFilterGenerator {
 public:
  ...
  /**
   * @brief Add physical-to-execution mapping information for deploying LIPFilters
   *        to a filter-injection node.
   *
   * @param filter_injection A physical FilterInjection node.
   * @param build_filter_operator_index The index of the BuildLIPFilterOperator
   *        that corresponds to \p filter_injection in the execution plan.
   */
  void addFilterInjectionInfo(const physical::FilterInjectionPtr &filter_injection,
                              const QueryPlan::DAGNodeIndex build_filter_operator_index) {
    builder_infos_.emplace_back(filter_injection, build_filter_operator_index);
    prober_infos_.emplace_back(filter_injection, build_filter_operator_index);
  }
